...
\hfill
\textcolor{blue}{\shadowbox{ Équations, inéquations et systèmes}}]
...
\begin{mybox}{Lycée Qualifiant Zitoun}{gray}
    ...
    \textcolor{darkbrown}{Durée totale : } $8h$
    \end{minipage}
\end{mybox}
...
\begin{itemize}
    \item Équations et inéquations du premier degré à une inconnue ;
    \item Équations et inéquations du second degré à une inconnue ;
    \begin{itemize}
        \item Forme canonique d'un trinôme ;
        \item Équations du second degré à une inconnue ;
    \end{itemize}
    \item Signe d'un trinôme du second degré ;
    \item Inéquations du second degré à une inconnue ;
    \item Les systèmes :
    \begin{itemize}
        \item Équations du premier degré à deux inconnues ;
        \item Systèmes de deux équations du premier degré à deux inconnues ;
        \item Régionnement du plan.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
   \item Résoudre des équations et des inéquations se ramenant à la résolution d'équations et d'inéquations du premier ou du second degré à une inconnue ;
    \item Résoudre un système de deux équations du premier degré à deux inconnues en utilisant différentes méthodes (combinaison linéaire, substitution, déterminant) ;
    \item Mathématiser, en utilisant des expressions, des équations, des inéquations, des inégalités ou des systèmes, une situation faisant intervenir des quantités variables ;
    \item Représenter graphiquement les solutions d'inéquations ou de systèmes d'inéquations du premier degré à deux inconnues, et utiliser cette représentation dans le régionnement du plan et dans la résolution de problèmes.
  \end{itemize}

{\Large\ding{45}}
\textcolor{red}{\large Recommandations pédagogiques :}
\begin{itemize}
	\item Les techniques de résolution des équations et inéquations du premier degré à une inconnue ont été étudiées au collège, il faudra renforcer cette pratique par l’étude de quelques exemples simples faisant intervenir la valeur absolue et les équations paramétriques simples, dans le but de développer la capacité des élèves à utiliser le raisonnement par disjonction des cas.
    \item Il faudra habituer les élèves à résoudre des équations du second degré sans recours au discriminant (racines évidentes, techniques de factorisation,…).
    \item Les équations et inéquations paramétriques du second degré sont hors programme ;
    \item Des problèmes, issus de la vie quotidienne ou des autres matières, devront être proposés dans le but d'habituer les élèves à mathématiser des situations et de les résoudre ;
\end{itemize}
\end{boxone}
\newpage
%\vspace{0.1cm}
\mysection{Équations du premier degré à une inconnue}

\textcolor{red}{Rappel 1:}

Résoudre dans $\mathbb{R}$ les équations suivants :
$$(E_1) : 2x - 3 = 0 \ \ ; \ \ (E_2) : 4x + 1 = 0\ \ ; \ \ (E_3) : 2x - 3 - 3(1-x) = 0$$ $$(E_4) : 2x - 1 - \dfrac{1}{2}(2 + x) = 4x - 9 \ \ ; \ \ (E_5) : 2x - 1 = 2(x + 1) \ \ ; \ \ (E_6) : \dfrac{x-1}{2} - \dfrac{2x + 5}{3} = 4 - \dfrac{x}{6}$$
$$(E_7) : (2x - 3)(\dfrac{3}{2}x + 5) = 0  \ \ ; \ \ (E_8) : (3x - 4)(5x + 1) - 2(3x - 4)(1 - x) = 0$$


\mysection{Inéquations du premier degré à une inconnue}

\textcolor{red}{Rappel 2:}

Résoudre dans $\mathbb{R}$ les inéquations suivants :
$$(I_1) : 2x - 6 \leq 0 \ \ ; \ \ (I_2) :4x +5 \geq 0 \ \ ; \ \ (I_3) : \dfrac{1}{2}x - 1 \leq 1 \ \ ; \ \ (I_4) : 3x + 1 \leq 0$$

\mysection{Signe du binôme $ax + b$ : }
\begin{Proposition}
On considère le binôme $ax + b$ où $a\neq 0$.
\begin{itemize}
	\item Si $x\geq -\dfrac{b}{a}$, alors $ax + b$ et $a$ ont le même signe.
	\item Si $x\leq -\dfrac{b}{a}$, alors $ax + b$ et $a$ sont de signes contraires.
\end{itemize}
\end{Proposition}

\textcolor{red}{Tableau de signe du binôme $ax + b$ :}
\begin{center}
	\begin{tikzpicture}
